...
const UpComing = ({navigation}) => {
  ...
  if (movies === false) {
    axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=f23a85ad3d65e712ab0822d95252d032&language=en-US&page=1`)
      .then((res) => {
        if (movies === false) {
   ...
   return (
    <ScrollView>
      <View style={{ backgroundColor: '#1a1818', paddingBottom: 20, paddingTop: 0, paddingLeft: 10}}>
        <Text style={{ fontSize: 21, color: 'white'}}>Up Coming</Text>
        <View >
          <FlatList
            data={movies}
            horizontal={true}
